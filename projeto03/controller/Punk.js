...
exports.getSingle = async (req,res) => { 
    ...
    await Punk.findById(req.params.id).then((Punk) => {
        res.status(200).json(Punk);
    }).catch((err) => {
...
exports.postCreate = async (req,res) => { 
    ...
    await Punk.create(req.body).then( () => {
...
exports.putUpdate = async (req,res) => {
    if(validaID(res,req.params.id)) return;
    if(validaEntrada(res,req.body)) return;
    await Punk.findByIdAndUpdate(req.params.id,req.body).then(() => {
...
exports.delDelete = async (req,res) => {
    if(validaID(res,req.params.id)) return;
    await Punk.findByIdAndDelete(req.params.id).then(() => {
