const HeavyMetal = require("./../model/HeavyMetal"); 
...
exports.getAll = async (req,res) => {
    await HeavyMetal.find({}).then((HeavyMetal) => { 
        res.status(200).json(HeavyMetal);
    }).catch((err) => {
...
exports.getSingle = async (req,res) => { 
    ...
    await HeavyMetal.findById(req.params.id).then((HeavyMetal) => {
        res.status(200).json(HeavyMetal);
    }).catch((err) => {
...
exports.postCreate = async (req,res) => { 
    ...
    await HeavyMetal.create(req.body).then( () => {
...
exports.putUpdate = async (req,res) => {
    if(validaID(res,req.params.id)) return;
    if(validaEntrada(res,req.body)) return;
    await HeavyMetal.findByIdAndUpdate(req.params.id,req.body).then(() => {
...
exports.delDelete = async (req,res) => {
    if(validaID(res,req.params.id)) return;
    await HeavyMetal.findByIdAndDelete(req.params.id).then(() => {
